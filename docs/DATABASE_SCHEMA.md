# Database Schema Design - Service Dog Standards

## Overview

This document defines the complete database schema for the Service Dog Standards application, designed to support multi-tenant architecture, comprehensive user management, service dog registry, e-commerce, and administrative functions.

## Core Principles

1. **Multi-tenant Support**: Organization-based isolation for different brands (SDS, ESA Standards, etc.)
2. **Role-based Access Control**: Handler, Trainer, Admin, Super Admin roles
3. **Audit Trail**: Created/updated timestamps on all entities
4. **Flexible Profile System**: JSON fields for extensible user profiles
5. **Comprehensive Relationships**: Dogs can have multiple trainers/handlers with different roles
...
- Organizations can have custom domains and subdomains
- Theme and settings isolation
- Data isolation by organization

### 2. Flexible User System
- Multiple roles with different permissions
- Public and private profiles
- Trainer-specific features (business info, custom URLs)
- Profile completion tracking

### 3. Comprehensive Dog Registry
- Unique registration numbers
- Multiple user relationships per dog
...
- Customizable items
- Cart management for anonymous users
...
-- Dog indexes
CREATE INDEX idx_dogs_registration_num ON dogs(registration_num);
CREATE INDEX idx_dogs_owner ON dogs(owner_id);
CREATE INDEX idx_dogs_org_status ON dogs(organization_id, status);
...
CREATE INDEX idx_dog_user_relationships_dog ON dog_user_relationships(dog_id);
CREATE INDEX idx_dog_user_relationships_user ON dog_user_relationships(user_id);
```

## Migration Considerations

1. **Data Migration**: Map existing MongoDB collections to new schema
2. **ID Migration**: Convert ObjectIDs to CUIDs or UUIDs
3. **JSON Fields**: Migrate complex nested data to JSON columns
4. **File URLs**: Update file storage paths and CDN URLs
5. **Relationships**: Rebuild foreign key relationships
6. **Indexes**: Create performance indexes for common queries

This schema provides a solid foundation for the Next.js application while maintaining data integrity and supporting all the complex business requirements of the Service Dog Standards platform.